...
export default function EditProductPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (prodError) {
        setError(getFriendlyErrorMessage(prodError));
      ...
      if (prodData) {
        setFormData({
          ...
          category_ids: prodData.category_ids || [],
          base_price: prodData.base_price.toString(),
          base_mrp: prodData.base_mrp ? prodData.base_mrp.toString() : "",
          badges: prodData.badges || [],
          image_url: prodData.image_url || "",
          benefits: prodData.benefits || "",
          how_to_use: prodData.how_to_use || "",
        });
      }
      setLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error: submitError } = await supabase.from("products").update({
        ...formData,
  ...
  if (loading) {
    return (
      <div className="flex min-h-[400px] items-center justify-center">
        <Loader2 className="animate-spin text-[#305724]" size={40} />
      </div>
    );
  }
  ...
  if (success) {
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center text-center">
        ...
        <h2 className="mt-4 text-2xl font-bold text-[#242424]">Product Updated!</h2>
        <p className="mt-2 text-[#5d6258]">Changes have been saved successfully.</p>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[#242424]">Edit Product</h1>
          <p className="text-sm text-[#5d6258]">Update multi-category and multi-badge data.</p>
        </div>
      </div>
